...
import com.wubydax.romcontrol.v3.MainActivity;
...
public class BackupRestoreIntentService extends IntentService {
    ...
    private void restoreValues(String filePath) {
        ...
        if (Objects.requireNonNull(filesInData).length > 0) {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                if (!line.equals("")) {
                    if (line.contains("###")) {
                        ...
                        Utils.putCommand("settings put system " + data[0] + " " + data[1]);
                    } else {
    ...
    private void iteratePrefsAndBackup() {
        ...
        for (File file : Objects.requireNonNull(allPrefFilesList)) {
    ...
    private void makeBackup(String backupString) {
        String currentDate = new SimpleDateFormat(getString(R.string.backup_file_prefix_date_format), Locale.ENGLISH)
                .format(Calendar.getInstance().getTime());
        String fileName = currentDate + "_" + Build.DISPLAY;
        String backupFileName = fileName + getString(R.string.backup_file_suffix);
        File newBackupFile = new File(Constants.BACKUP_FOLDER_PATH + File.separator +
                backupFileName);
        try {
            ...
            intent.putExtra(Constants.BACKUP_FILE_PATH_EXTRA_KEY, String.format(Locale.getDefault(),
                    getString(R.string.backup_complete_toast), newBackupFile.getAbsoluteFile()));
