...
public class ColorPickerPreference extends Preference implements
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            mAlphaSliderEnabled = attrs.getAttributeBooleanValue(null,
                    "alphaSlider", true);
            mHexValueEnabled = attrs.getAttributeBooleanValue(null,
                    "hexValue", true);
        }
    }
    ...
    private Bitmap getPreviewBitmap() {
        int d = Math.round(getContext().getResources().getDimension(R.dimen.button_size));
        int color = mValue;
        ...
        for (int i = 0; i < w; i++) {
            for (int j = i; j < h; j++) {
                int c = i <= 1 || i >= w - 2 || j >= h - 2 ? color : color;
                bm.setPixel(i, j, c);
                if (i != j) {
                    bm.setPixel(j, i, c);
                }
    ...
    @Override
    public void onColorChanged(int color, boolean put) {
        if (put) {
            Utils.putKey(getKey() + " " + color);
        }
        mValue = color;
