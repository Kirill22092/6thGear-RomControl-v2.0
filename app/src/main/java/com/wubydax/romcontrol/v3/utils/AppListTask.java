...
public class AppListTask extends AsyncTask<Void, Void, List<AppInfo>> {
    ...
    protected List<AppInfo> doInBackground(Void... voids) {
        ...
        for (ResolveInfo resolveInfo : resolveInfoList) {
            ...
            explicitIntent.setComponent(new ComponentName(appInfo.mPackageName, resolveInfo
                    .activityInfo.name));
            appInfo.mIntent = explicitIntent;
            appList.add(appInfo);
        }
        Collections.sort(appList, (lhs, rhs) -> String.CASE_INSENSITIVE_ORDER.compare(lhs.mAppName,
                rhs.mAppName));
        return appList;
    }
